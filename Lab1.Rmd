...
OK, now let's read the data into R. Paste the link from the *Online Distribution Info* into the read.csv function, and set the header argument to True. 

What does header = T mean? 
Should we create an object here? Why or why not? 
```{r}
t_air <- read.csv("https://cn.dataone.org/cn/v2/resolve/urn:uuid:3e962856-7ece-4ff8-870a-82511c31c974",
                   header = T)
```
...
OK, now that we know some of the basics of R let's see if we can apply it to things we've been learning. 
...
Save the script as 'lastname_lab1.R' in your lab1 folder on the network by next Wednesday. 
